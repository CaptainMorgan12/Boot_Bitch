...
git add -A
git status
git commit -m "Prepare Boot Bitch 0.2.24 release"
...
Create the 0.2.24 release after the package, helper, EFI reconciliation, and
...
git status
git log -1 --oneline
git tag -a v0.2.24 -m "Boot Bitch 0.2.24 firmware destination maintenance"
git push origin v0.2.24
gh release create v0.2.24 \
  /home/amiga/Projects/Boot_Repair/build-release/boot-repair_0.2.24_amd64.deb \
  /home/amiga/Projects/Boot_Repair/build-release/boot-repair_0.2.24_x86_64.AppImage \
  /home/amiga/Projects/Boot_Repair/Development/release-0.2.24/boot-bitch-0.2.24-1-x86_64.pkg.tar.zst \
  /home/amiga/Projects/Boot_Repair/Development/release-0.2.24/SHA256SUMS \
  docs/release-notes-0.2.24.md \
  --verify-tag \
  --title "Boot Bitch 0.2.24" \
  --notes-file docs/release-notes-0.2.24.md
```

The release assets are attached to GitHub Releases and are not copied into the source tree.
Generate SHA256SUMS from the final Debian package, AppImage, Arch package and
release notes, using asset basenames so downloaded files can be verified with
